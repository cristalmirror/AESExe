#pragma once
#include "ICipher.hpp"
#include <vector>
#include <algorithm>

class AES128 : public ICipher {
public:
    AES128(const std::vector<uint8_t>& key) {
        expandKey(key);
    }

    void encryptBlock(const uint8_t* in, uint8_t* out) override {
        uint8_t state[16];
        std::copy(in, in + 16, state);
        addRoundKey(state, roundKeys[0].data());
        for (int i = 1; i <= 9; ++i) {
            subBytes(state);
            shiftRows(state);
            mixColumns(state);
            addRoundKey(state, roundKeys[i].data());
        }
        subBytes(state);
        shiftRows(state);
        addRoundKey(state, roundKeys[10].data());
        std::copy(state, state + 16, out);
    }

    void decryptBlock(const uint8_t* in, uint8_t* out) override {
        uint8_t state[16];
        std::copy(in, in + 16, state);
        addRoundKey(state, roundKeys[10].data());
        invShiftRows(state);
        invSubBytes(state);
        for (int i = 9; i >= 1; --i) {
            addRoundKey(state, roundKeys[i].data());
            invMixColumns(state);
            invShiftRows(state);
            invSubBytes(state);
        }
        addRoundKey(state, roundKeys[0].data());
        std::copy(state, state + 16, out);
    }

    size_t getBlockSize() const override { return 16; }

private:
    std::vector<std::vector<uint8_t>> roundKeys;
    static const uint8_t sbox[256];
    static const uint8_t invSbox[256];

    void subBytes(uint8_t* s) { for (int i = 0; i < 16; ++i) s[i] = sbox[s[i]]; }
    void invSubBytes(uint8_t* s) { for (int i = 0; i < 16; ++i) s[i] = invSbox[s[i]]; }

    void shiftRows(uint8_t* s) {
        uint8_t t[16];
        t[0] = s[0]; t[4] = s[4]; t[8] = s[8]; t[12] = s[12];
        t[1] = s[5]; t[5] = s[9]; t[9] = s[13]; t[13] = s[1];
        t[2] = s[10]; t[6] = s[14]; t[10] = s[2]; t[14] = s[6];
        t[3] = s[15]; t[7] = s[3]; t[11] = s[7]; t[15] = s[11];
        std::copy(t, t + 16, s);
    }

    void invShiftRows(uint8_t* s) {
        uint8_t t[16];
        t[0] = s[0]; t[4] = s[4]; t[8] = s[8]; t[12] = s[12];
        t[1] = s[13]; t[5] = s[1]; t[9] = s[5]; t[13] = s[9];
        t[2] = s[10]; t[6] = s[14]; t[10] = s[2]; t[14] = s[6];
        t[3] = s[7]; t[7] = s[11]; t[11] = s[15]; t[15] = s[3];
        std::copy(t, t + 16, s);
    }

    uint8_t xtime(uint8_t x) { return (x << 1) ^ ((x & 0x80) ? 0x1b : 0x00); }

    void mixColumns(uint8_t* s) {
        for (int i = 0; i < 4; ++i) {
            uint8_t* c = s + i * 4;
            uint8_t s0 = c[0], s1 = c[1], s2 = c[2], s3 = c[3];
            c[0] = xtime(s0) ^ (xtime(s1) ^ s1) ^ s2 ^ s3;
            c[1] = s0 ^ xtime(s1) ^ (xtime(s2) ^ s2) ^ s3;
            c[2] = s0 ^ s1 ^ xtime(s2) ^ (xtime(s3) ^ s3);
            c[3] = (xtime(s0) ^ s0) ^ s1 ^ s2 ^ xtime(s3);
        }
    }

    uint8_t gmul(uint8_t a, uint8_t b) {
        uint8_t p = 0;
        for (int i = 0; i < 8; i++) {
            if (b & 1) p ^= a;
            bool hi = (a & 0x80);
            a <<= 1;
            if (hi) a ^= 0x1b;
            b >>= 1;
        }
        return p;
    }

    void invMixColumns(uint8_t* s) {
        for (int i = 0; i < 4; i++) {
            uint8_t* c = s + 4 * i;
            uint8_t s0 = c[0], s1 = c[1], s2 = c[2], s3 = c[3];
            c[0] = gmul(s0, 0x0e) ^ gmul(s1, 0x0b) ^ gmul(s2, 0x0d) ^ gmul(s3, 0x09);
            c[1] = gmul(s0, 0x09) ^ gmul(s1, 0x0e) ^ gmul(s2, 0x0b) ^ gmul(s3, 0x0d);
            c[2] = gmul(s0, 0x0d) ^ gmul(s1, 0x09) ^ gmul(s2, 0x0e) ^ gmul(s3, 0x0b);
            c[3] = gmul(s0, 0x0b) ^ gmul(s1, 0x0d) ^ gmul(s2, 0x09) ^ gmul(s3, 0x0e);
        }
    }

    void addRoundKey(uint8_t* s, const uint8_t* key) {
        for (int i = 0; i < 16; ++i) s[i] ^= key[i];
    }

    void expandKey(const std::vector<uint8_t>& key) {
        roundKeys.assign(11, std::vector<uint8_t>(16));
        std::vector<uint8_t> exp(176);
        std::copy(key.begin(), key.end(), exp.begin());
        uint8_t rcon = 0x01;
        for (int i = 16; i < 176; i += 4) {
            std::vector<uint8_t> temp(exp.begin() + i - 4, exp.begin() + i);
            if (i % 16 == 0) {
                std::rotate(temp.begin(), temp.begin() + 1, temp.end());
                for (auto& b : temp) b = sbox[b];
                temp[0] ^= rcon;
                rcon = xtime(rcon);
            }
            for (int j = 0; j < 4; ++j) exp[i + j] = exp[i - 16 + j] ^ temp[j];
        }
        for (int i = 0; i < 11; ++i)
            std::copy(exp.begin() + i * 16, exp.begin() + i * 16 + 16, roundKeys[i].begin());
    }
};

const uint8_t AES128::sbox[256] = {
    
};

const uint8_t AES128::invSbox[256] = {
    
};
